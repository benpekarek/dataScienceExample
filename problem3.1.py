...
@author: Ben
"""
#reusing past homework to calculate linear regression for problem 3
import numpy as np
...
def main(datapath, degrees):
    ...
    for row in filtered_bike:
        xcol.append(float(row[-1]))
        ycol.append(float(row[4]))
    ...
    plt.plot(t, paramFits[0][1] + paramFits[0][0] *t, 'c--', t, paramFits[1][2] + paramFits[1][1] *t + paramFits[1][0]* t**2, 'g--')
    
    plt.legend(['1st degree','2nd degree', 'data'])
    plt.xlabel('Total traffic')
    plt.ylabel('Precipitation')
    plt.title('Precipitation vs. Total Traffic')
    plt.show()
    print(paramFits)
